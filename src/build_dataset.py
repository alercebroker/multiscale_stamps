...
def extract_ghost_bogus():
    ...
    dataset_javier = pd.read_pickle(
        'data/training_set_Aug-07-2020-oid-class_jarredondo.pkl')
    ...
    dataset_ereyes = pd.read_pickle(
        'data/training_set_Jun-22-2020.pkl')
    ...
    ghost_bogus_ereyes.to_pickle('data/ghost_bogus.pkl')


def curated_bogus_list():
    """ List of known bogus objects in ALeRCE's database."""
    training_set = pd.read_pickle(
        'data/dfcrossmatches_ZTF_prioritized_v8.0.3.pickle')
    dataset_javier = pd.read_pickle(
        'data/training_set_Aug-07-2020-oid-class_jarredondo.pkl')
    bogus_javier = dataset_javier[dataset_javier['class'] == 'bogus'].drop_duplicates()
    ghost_bogus = pd.read_pickle('data/ghost_bogus.pkl')
    ...
    mislabeled_bogus = pd.read_csv(
        'satellites/pearl_necklace_tagged_as_bogus.txt',
    ...
    long_bogus = pd.read_csv(
        'data/verified_long_bogus.txt', comment='#', header=None)
    ...
    curated_bogus.to_csv(
        'data/curated_bogus.txt', index=False, header=False)
...
def curated_asteroid_list():
    ...
    curated_asteroid.to_csv(
        'data/curated_asteroid.txt', index=False, header=False)


def download_bogus():
    curated_bogus = pd.read_csv('data/curated_bogus.txt', header=None)
    download_stamps(curated_bogus[0].values, 'data/database_bogus.pkl')


def download_sne():
    training_set = pd.read_pickle('data/dfcrossmatches_ZTF_prioritized_v8.0.3.pickle')
    sne_classes = ['SNIa', 'SNII', 'SNIbc', 'SNIIn', 'SLSN', 'SNIIb']
    sne_list = training_set[training_set['classALeRCE'].isin(sne_classes)]
    download_stamps(sne_list.index.unique().values, 'data/sne_stamps.pkl')
...
def download_vs():
    ...
    download_stamps(vs_sample.index.unique().values, 'data/vs_stamps.pkl')
...
def download_agn():
    ...
    download_stamps(agn_sample.index.unique().values, 'data/agn_stamps.pkl')


def download_asteroid():
    curated_asteroid = pd.read_csv('data/curated_asteroid.txt', header=None)
    download_stamps(curated_asteroid[0].values, 'data/asteroid_stamps.pkl')
...
def download_satellites():
    satellites = pd.read_csv('satellites/bogus_moving_v1.0.0.csv')
    mislabeled_bogus = pd.read_csv(
        'satellites/pearl_necklace_tagged_as_bogus.txt',
        comment='#', header=None, names=['oid']
    )
...
def consolidate_datasets():
    sne = pd.read_pickle('data/sne_stamps.pkl')
    sne['class'] = 'sn'

    vs = pd.read_pickle('data/vs_stamps.pkl')
    vs['class'] = 'vs'

    agn = pd.read_pickle('data/agn_stamps.pkl')
    agn['class'] = 'agn'
    ...
    database_bogus = pd.read_pickle('data/database_bogus.pkl')
    database_bogus['class'] = 'bogus'

    asteroid = pd.read_pickle('data/asteroid_stamps.pkl')
    asteroid['class'] = 'asteroid'

    satellite = pd.read_pickle('data/satellites.pkl')
    ...
    dataset.to_pickle('data/consolidated_dataset.pkl')
