...
def build_ndarrays(seed: int):
    consolidated_dataset_df = pd.read_pickle(os.path.join(PROJECT_DIR, 'data/consolidated_dataset.pkl'))
    ...
    with open('data/consolidated_ndarrays.pkl', 'wb') as f:
        pickle.dump(
...
def process_ndarrays():
    with open('data/consolidated_ndarrays.pkl', 'rb') as f:
        d = pickle.load(f)
    ...
    with open('data/normalized_ndarrays.pkl', 'wb') as f:
        pickle.dump(
            d, f, protocol=pickle.HIGHEST_PROTOCOL
        )
...
def get_tf_datasets(dataset_name: str, batch_size: int):
    if dataset_name == 'multiscale':
        d = pd.read_pickle(os.path.join(PROJECT_DIR, 'data/multiscale_stamps.pkl'))
    elif dataset_name == 'cropped':
        d = pd.read_pickle(os.path.join(PROJECT_DIR, 'data/cropped_stamps_16.pkl'))
    elif dataset_name == 'low_res':
        d = pd.read_pickle(os.path.join(PROJECT_DIR, 'data/low_res_stamps_16.pkl'))
    elif dataset_name == 'full':
        d = pd.read_pickle(os.path.join(PROJECT_DIR, 'data/normalized_ndarrays.pkl'))
    else:
    ...
    return training_dataset, validation_dataset, test_dataset, label_encoder
